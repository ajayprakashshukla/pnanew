         <!DOCTYPE html>
          ...
          <body>
               <div style="width: 700px;margin: 0px auto;border: 1px solid red;padding: 26px;">
                   ...
                   <img width="150px" src="{{url('public/assets/pages/img/logo-big-white.png')}}">
                   </header>

                   <table width="100%">
                       <tr>
                        <td>Date: <span style="width: 300px;border-bottom: 1px solid #000;">{{date('Y-m-d')}}</span></td>
                       </tr>
                       <tr>
                       ...
                       <tr>
                        <td>I, {{ $user->name}} {{ $user->middle_name}} {{ $user->lastname}}  -SHARP NO: {{ $user->sharp_id}} have obtained a loan of {{number_format($loan_application->amount_requested, 2, ',', ',') }} (NGN{{number_format($loan_application->amount_requested, 2, ',', ',') }}), and I hereby authorize you to deduct from my monthly salary with effect from the month of {{date('M',strtotime($repayment_plan->date))}} {{date('Y',strtotime($repayment_plan->date))}} the amounts indicated in the attached payback plan and remit same to Progressive Mobil Employees Multi-purpose Cooperative Society Limited through an account as advised by the Society.</td>
                       </tr>
                       <tr>

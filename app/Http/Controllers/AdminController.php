...
public function ImportData(){
 ...
 function getCustomersDearData(){
	  $data= $this->curlRequest(['method'=>'get','end_point'=>'/Customers']);

      if($data->Total){
			
			$total_pages=ceil((int)$data->Total/100);
			
		for($i=1;$i<=$total_pages;$i++){	
		   $newdata= $this->curlRequest(['method'=>'get','end_point'=>'/Customers?page='.$i]);	
          foreach ($newdata->Customers as $Customer):
    ...
    public function getProductsDearData(){
        $datas= $this->curlRequest(['method'=>'get','end_point'=>'/Products']);
       $total_pages=ceil((int)$datas->Total/100);
	   
	   for($i=1;$i<=$total_pages;$i++){
			$newdatas= $this->curlRequest(['method'=>'get','end_point'=>'/Products?page='.$i]);
			
			foreach ($newdatas->Products as $data):
